...
\end{tikzpicture}
\caption{Wheatstone bridge configurations. Sensitivity doubles with each step (Figure~\ref{fig:bridge}): quarter-bridge ($1$ active element), half-bridge\index{bridge!half} ($2$), full-bridge\index{bridge!full} ($4$). Full bridge also provides complete temperature compensation.}
\label{fig:bridge}
\end{figure}
