...
\begin{equation}
\Delta V_o \approx \frac{V_{ex}}{4} \cdot \frac{\Delta R}{R} \quad \text{(quarter-bridge)}\footnote{This is the linearized approximation, valid for $\Delta R / R \ll 1$ (i.e., strains below ${\sim}5000\,\mu\epsilon$). The exact expression is $\Delta V_o = V_{ex} \cdot (\Delta R/R) / (4 + 2\Delta R/R)$, which matters for large strains or for high-accuracy absolute strain measurement.}
\end{equation}
