...
\node[step] (spec) at (0,0) {Define\\Specifications};
\node[dec] (phase) at (3.5,0) {Linear phase\\required?};
\node[step] (fir) at (7,1.2) {FIR Design\\(window method)};
\node[step] (iir) at (7,-1.2) {IIR Design\\(Butterworth/Cheby)};
\node[step] (impl) at (10.5,0) {Implement in\\LabVIEW};
\node[step] (verify) at (13.5,0) {Verify with\\known signal};
\draw[arr] (spec) -- (phase);
