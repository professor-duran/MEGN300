...
\textbf{Module~8D / Design Challenge}: Read before the integration phase of the design challenge. The debugging pyramid and signal walk protocol are your primary tools when things do not work. Return to this chapter any time a lab produces unexpected results. \textbf{Related}: Circuit fundamentals in Chapter~\ref{ch:circuits} (p.\,\pageref{ch:circuits}); SNR and grounding in Chapter~\ref{ch:snr} (p.\,\pageref{ch:snr}).
\end{tipbox}
