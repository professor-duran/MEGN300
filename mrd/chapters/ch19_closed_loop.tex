...
\textbf{Module~9 (Weeks 13--14)}: Read after the open-loop chapter. Focus on feedback block diagrams, stability concepts (gain/phase margin), and the step response characteristics (overshoot, settling time). \textbf{Related}: Open-loop in Chapter~\ref{ch:openloop} (p.\,\pageref{ch:openloop}); PID tuning in Chapter~\ref{ch:pid} (p.\,\pageref{ch:pid}).
\end{tipbox}
