...
(2) \textbf{Integral windup}: when the actuator saturates, the integral continues to accumulate, causing massive overshoot when the error changes sign. This is addressed in Chapter~\ref{ch:pid}.
...
For the ThermoRig ramping at 1\,\degC{}/s with PI control ($K_v = K_i K_{\text{plant}} = 0.5 \times 1.1 = 0.55$\,s$^{-1}$): $e_{ss} = 1.0/0.55 = 1.82$\,\degC{}. The temperature always lags the ramp by 1.82\,\degC{}. Increasing $K_i$ reduces this lag but increases overshoot at the end of the ramp.
