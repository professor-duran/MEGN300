...
\node[amp, draw=AccentTeal, fill=AccentTeal!8] (ina) at (12,0) {\textbf{INA}\\$G = 1 + 49.4\text{k}/R_G$\\Highest CMRR\\Input $Z$: $>$G$\Omega$};
% Best for labels
...
A complete signal conditioning chain (Figure~\ref{fig:signal_chain}) for a resistive sensor:
...
Sensor outputs are often too small for the ADC. A Type K thermocouple at 100\,\degC{} produces only 4.1\,mV. An NI USB-6001 on $\pm$10\,V has 1.22\,mV LSB, meaning the TC signal spans only 3--4 bits. Amplification by 500--1000$\times$ uses the full 14-bit range.
...
TC produces 0--8.2\,mV (0--200\,\degC{}). DAQ accepts 0--5\,V. Required gain: $G \approx 600$. INA128: $R_G = 49.4\,\text{k}\Omega/(600-1) = 82.5\,\Omega$. Use $82\,\Omega$ standard (E24), giving $G = 1 + 49400/82 = 603$. CMRR of 120\,dB rejects 50\,mV ground offset. Signal chain: TC $\to$ INA128 ($G = 603$) $\to$ Sallen-Key LP (40\,Hz) $\to$ NI USB-6001 (diff, 0--5\,V, 100\,S/s).
\end{examplebox}
...
\textbf{Electrical characteristics table}: input offset voltage\index{offset voltage} ($V_{os}$), input bias current ($I_b$), open-loop gain ($A_{ol}$), CMRR, GBW product, slew rate\index{slew rate}. These are specified at particular conditions (supply voltage, temperature).
...
\textbf{Step 4: Design the anti-alias filter.} For 100\,S/s sample rate and 10\,Hz signal bandwidth: 2nd-order Sallen-Key Butterworth at $f_c = 40$\,Hz. This provides $-5.4$\,dB attenuation at $f_s/2 = 50$\,Hz, adequate for 14-bit.
...
For a TL072 ($V_{os} = 3$\,mV typical) at gain 603 (ThermoRig INA configuration): $V_{\text{output, offset}} = 3 \times 10^{-3} \times 603 = 1.81$\,V. The thermocouple signal at 50\,\degC{} is $50 \times 0.041 = 2.05$\,mV, which amplified to $2.05 \times 603 = 1.24$\,V. The offset (1.81\,V) is \emph{larger than the signal} (1.24\,V). The measurement is useless.
...
\item Select and configure an INA128 for a bridge circuit ($G = 1 + 49.4\,\text{k}\Omega/R_G$).
