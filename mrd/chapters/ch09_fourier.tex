...
\end{scope}
\end{tikzpicture}
\caption{Fourier spectra of common periodic waveforms. Square and triangle waves contain only odd harmonics; sawtooth contains all harmonics. The roll-off rate determines how many harmonics carry significant energy, which in turn determines the bandwidth required to faithfully measure the signal.}
...
Every real signal is a superposition of sinusoids. If your measurement system attenuates or phase-shifts any frequency component differently from others, the reconstructed signal is distorted. Understanding Fourier decomposition lets you predict how your sensor's frequency response (Chapter~\ref{ch:dynamic}) and your filters (Chapters~\ref{ch:analog_filters}--\ref{ch:digital_filters}) will affect the measured signal.
\end{tipbox}
...
The Fourier series applies to periodic signals. The Fourier transform\index{Fourier transform} generalizes to non-periodic signals (transients, events) by letting the period approach infinity. The discrete version (DFT/FFT) is the computational tool used in digital systems (Chapter~\ref{ch:fft}).
