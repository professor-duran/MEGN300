...
\textbf{Module~4 (Weeks 5--6)}: Read the full chapter before the ADC/sampling lab. The sampling theorem, quantization error, and NI DAQ configuration sections are directly applied in that lab. \textbf{Related}: SNR in Chapter~\ref{ch:snr} (p.\,\pageref{ch:snr}); DSP fundamentals in Chapter~\ref{ch:dsp} (p.\,\pageref{ch:dsp}).
\end{tipbox}
...
\end{equation}
At high frequencies, jitter becomes the dominant noise source, limiting effective resolution. For the low-frequency measurements typical of the USB-6001 ($<$1\,kHz), clock jitter is negligible compared to quantization noise and is not a practical concern.
