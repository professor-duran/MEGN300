...
\textbf{Module~6 (Weeks 9--10)}: Read the aliasing and anti-aliasing sections before the filter design lab. The Nyquist theorem and its practical implications (always filter before sampling) are foundational. \textbf{Related}: ADC settings in Chapter~\ref{ch:analog_digital} (p.\,\pageref{ch:analog_digital}); analog filter design in Chapter~\ref{ch:analog_filters} (p.\,\pageref{ch:analog_filters}); digital filter design in Chapter~\ref{ch:digital_filters} (p.\,\pageref{ch:digital_filters}).
\end{tipbox}
