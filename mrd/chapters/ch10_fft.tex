...
\end{axis}
\end{tikzpicture}
\caption{Spectral leakage (Figure~\ref{fig:leakage}): without windowing (left), a 60.5\,Hz signal smears across many FFT bins. With a Hanning window (right), sidelobe energy is suppressed and the peak sharpens. Always window unless the signal is exactly periodic in the acquisition window.}
\label{fig:leakage}
\end{figure}
