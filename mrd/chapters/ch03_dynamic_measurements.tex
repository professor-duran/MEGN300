...
\addplot[domain=0:15, samples=300, thick, MinesSilver!70!black]
    {1 - 1.077*exp(-0.268*x) + 0.077*exp(-3.732*x)};
...
In a feedback control system, the sensor is part of the loop. Its dynamic response affects stability. A sensor with time constant $\tau_s$ introduces phase lag $\phi = -\arctan(\omega \tau_s)$ at the crossover frequency. If this lag reduces the total phase margin below \ang{45}, the system becomes oscillatory or unstable.
...
For phase angle: measure the time delay $\Delta t$ between the input and output waveforms (zero-crossing to zero-crossing, or peak to peak). The phase angle in degrees is $\phi = -360 \cdot f \cdot \Delta t$. For a first-order system, $\phi$ is always negative (output lags input) and ranges from \ang{0} at DC to $-$\ang{90} at infinite frequency. For a second-order system, $\phi$ ranges from \ang{0} to $-$\ang{180}, passing through $-$\ang{90} at the natural frequency.
...
\textbf{Example 2: Pressure measurement in a hydraulic actuator.} The pressure oscillates at 50\,Hz due to pump ripple. A pressure transducer with $f_n = 500$\,Hz ($\zeta = 0.7$) has an amplitude ratio of $M = 1/\sqrt{(1-0.01)^2 + (0.14)^2} = 1.0002$ at 50\,Hz, practically perfect (0.02\% error). A cheaper transducer with $f_n = 100$\,Hz ($\zeta = 0.4$) has $M = 1/\sqrt{(1-0.25)^2 + (2 \times 0.4 \times 0.5)^2} = 1.18$ at 50\,Hz, overstating the pressure by 18\%. At 80\,Hz, the resonance amplification makes the reading meaningless.
...
\textbf{Dynamic range}: $20\log_{10}(50/0.003) = 84$\,dB ($\sim$14 bits equivalent). The DAQ must have at least this dynamic range to use the sensor's full capability. A 12-bit ADC (72\,dB) would limit the usable range; a 16-bit ADC (96\,dB) provides headroom.
