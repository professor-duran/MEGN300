...
\textbf{Module~9 (Weeks 13--14)}: Read before the PID tuning lab. Focus on P/I/D individual effects, Ziegler-Nichols tuning, and integral windup. The ThermoRig PID example is directly applicable to the lab. \textbf{Related}: Closed-loop fundamentals in Chapter~\ref{ch:closedloop} (p.\,\pageref{ch:closedloop}); high-power output stages in Chapter~\ref{ch:highpower} (p.\,\pageref{ch:highpower}).
\end{tipbox}
