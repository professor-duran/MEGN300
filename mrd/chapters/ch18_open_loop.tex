...
\label{ch:openloop}\index{control!open-loop}\index{feedforward control}\index{control!feedforward}
...
From this step response, extract three parameters:
\begin{itemize}[nosep,leftmargin=1.5em]
\item \textbf{Process gain}: $K_p = (T_{ss} - T_{\text{ambient}}) / (\text{duty cycle change})$
\item \textbf{Time constant}: $\tau$ (time to reach 63.2\% of $T_{ss} - T_{\text{ambient}}$)
\item \textbf{Dead time}: $L$ (time from step to first observable temperature rise)
\end{itemize}
For the ThermoRig: $K_p \approx 1.1$\,\degC{}/\%, $\tau \approx 30$\,s, $L \approx 3$\,s.

These three parameters ($K_p$, $\tau$, $L$) define a First-Order Plus Dead Time (FOPDT\index{FOPDT model}) model: 
\[
G(s) = \frac{K_p \, e^{-Ls}}{1 + \tau s}
\]
This First-Order Plus Dead Time (FOPDT) model is the most common plant model used for PID tuning in process control.
...
For the ThermoRig: to ramp the temperature from 25\,\degC{} to 80\,\degC{} in exactly 60\,s with a linear ramp: the desired trajectory is $T(t) = 25 + 0.917t$ for $0 \leq t \leq 60$\,s. The required heater power includes both the steady-state loss compensation and the transient power to heat the thermal mass: 
\[
P(t) = hA(T(t) - T_{\text{amb}}) + mC_p \cdot dT/dt = hA(25 + 0.917t - 22) + 30.6 \times 0.917
\]
The second term is a constant 28\,W needed to maintain the ramp rate. The first term increases linearly as the temperature rises.
