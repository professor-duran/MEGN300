...
\textbf{Module~3 (Weeks 3--4)}: Read alongside Chapter~4. Focus on $t$-test\index{$t$-test}s, confidence intervals, and outlier detection (Chauvenet\index{Chauvenet criterion}'s criterion). These are applied in every lab report for the rest of the semester. \textbf{Related}: Error analysis in Chapter~\ref{ch:error} (p.\,\pageref{ch:error}).
\end{tipbox}
...
The \textbf{median} is the middle value when data are sorted. It is robust to outliers: a single extreme measurement shifts the mean but barely affects the median. Use the median when you suspect outliers or when the data distribution is skewed.
...
The \textbf{sample standard deviation\index{standard deviation}} $s = \sqrt{(1/(N-1))\sum(x_i - \bar{x})^2}$ quantifies the typical scatter of individual measurements. It has the same units as the data.
...
The most important distribution in measurement science (Figure~\ref{fig:normal_dist}). Random errors from many independent sources combine to produce a normal distribution (Central Limit Theorem). Properties: symmetric, bell-shaped, fully characterized by mean $\mu$ and standard deviation $\sigma$. 68.3\% of data falls within $\pm 1\sigma$, 95.4\% within $\pm 2\sigma$, 99.7\% within $\pm 3\sigma$.
...
The distribution of the sample variance $s^2$ scaled by $(N-1)/\sigma^2$. Used to construct confidence intervals for the variance itself and for goodness-of-fit testing. In MEGN~300, you encounter it when assessing whether the scatter in your calibration data are consistent with the claimed sensor accuracy.
...
\subsection{p-Values}
The $p$-value\index{$p$-value} is the probability of observing a test statistic at least as extreme as the one computed, assuming the null hypothesis is true. Convention: $p < 0.05$ (5\% significance level) is considered statistically significant. $p < 0.01$ is highly significant. $p > 0.1$ is not significant.
...
\section{Analysis of Variance (ANOVA\index{ANOVA})}
...
Laboratory data are often collected as a time series (sequential measurements over time). Beyond the standard statistics above, time series data has temporal structure:

\textbf{Trend}: a long-term drift in the mean (sensor aging, environmental change). Detected by: linear regression\index{regression!linear} on the time series, or comparing means of the first and last thirds of the data.
