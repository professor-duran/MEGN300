...
\label{ch:analog_filters}\index{filter!low-pass}\index{filter!Butterworth}\index{filter!Chebyshev}\index{filter!Bessel}\index{cutoff frequency}\index{Sallen-Key topology}
...
\addplot[domain=0.1:100, samples=200, thick, AccentTeal] {-10*log10(1+x^4)};
...
Digital filters (Chapter~\ref{ch:digital_filters}) operate on digitized signals only. Before the ADC, the only noise removal tool is an analog filter. The anti-alias filter (analog LP before ADC) is mandatory; without it, aliasing permanently corrupts digital data.

An alternative design procedure that starts from a target cutoff frequency and uses equal-component values is presented below. Both approaches yield valid Sallen-Key Butterworth filters; choose whichever fits your design constraints.
...
(1) Determine $f_{\max}$ and $f_s$. (2) Set $f_c$ between $f_{\max}$ and $f_s/2$ (common: $f_c = f_s/4$). (3) Required stopband attenuation at $f_s/2$: $\geq 6.02n + 1.76$\,dB for $n$-bit ADC. (4) Choose filter type. (5) Calculate order: $n \geq \text{attenuation}/(20\log_{10}(f_s/(2f_c)))$ for Butterworth. (6) Implement with cascaded Sallen-Key stages. (7) Measure actual response.
...
\textbf{Step 4: Calculate required order.} The Butterworth magnitude response at frequency $f$: $|H(f)| = 1/\sqrt{1 + (f/f_c)^{2n}}$. Attenuation at $f_N = 1000$\,Hz: $A = 20n\log_{10}(1000/400) = 20n \times 0.398 = 7.96n$\,dB. For 86\,dB: $n \geq 86/7.96 = 10.8$. Use $n = 12$ (next even integer, implemented as six cascaded Sallen-Key stages).

\textbf{Step 5: Compute component values.} Each Sallen-Key stage has specific $Q$ values determined by the Butterworth polynomial. For a 12th-order Butterworth, the six second-order section $Q$ values are: $Q_1 = 0.504$, $Q_2 = 0.522$, $Q_3 = 0.566$, $Q_4 = 0.654$, $Q_5 = 0.841$, $Q_6 = 1.932$. For each section with $f_c = 400$\,Hz: choose $C = 10$\,nF (convenient standard value). Then $R = 1/(2\pi f_c C) = 1/(2\pi \times 400 \times 10^{-8}) = 39.8$\,k$\Omega$. Use 39.2\,k$\Omega$ (nearest E96 value). The gain for each section: $G = 3 - 1/Q$. For $Q_6 = 1.932$: $G = 2.482$, so $R_f/R_g = 1.482$. Use $R_f = 14.7$\,k$\Omega$, $R_g = 10.0$\,k$\Omega$ (ratio $= 1.47$, close enough with 1\% resistors).
...
\item A second-order Sallen-Key low-pass filter must have a Butterworth response with $f_c = 500$\,Hz. If you choose $C_1 = C_2 = 10$\,nF, calculate the required resistor values $R_1$ and $R_2$. Verify that your design achieves the correct cutoff frequency.
\item You are sampling a sensor signal at $f_s = 10$\,kS/s and need an anti-aliasing filter that provides at least $-60$\,dB of attenuation at the Nyquist frequency ($5$\,kHz). The signal bandwidth of interest is 0--500\,Hz. What is the minimum Butterworth filter order required? Would you choose Butterworth, Chebyshev, or Bessel for this application, and why?
\item Sketch the Bode magnitude and phase plots for the transfer function $H(s) = \dfrac{1000}{s + 1000}$. Label the $-3$\,dB frequency, the passband gain, the high-frequency roll-off rate, and the phase at $f_c$.
\item A first-order RC low-pass filter has $R = 10\,\text{k}\Omega$ and $C = 100$\,nF. (a) Calculate the $-3$\,dB cutoff frequency. (b) Calculate the time constant $\tau$. (c) If the input is a 1\,V step, how long does the output take to reach 0.63\,V? (d) What is the attenuation in dB at $10 f_c$?
\item For each scenario, state whether a passive or active filter is more appropriate and explain your reasoning: (a) Anti-aliasing filter for a battery-powered portable data logger. (b) A filter that must provide gain while filtering. (c) A 4th-order low-pass for a benchtop instrument with $\pm$15\,V supplies available.
\item A noise source at 2\,kHz must be attenuated by at least $-40$\,dB, but the signal of interest extends up to 500\,Hz with no more than $-1$\,dB of passband attenuation allowed. Determine the minimum filter order for (a) a Butterworth response and (b) a Chebyshev Type~I response with 0.5\,dB passband ripple. Which would you recommend and why?
\end{enumerate}
...
\item For a Butterworth Sallen-Key with equal capacitors ($C_1 = C_2 = C = 10$\,nF), the cutoff frequency is $f_c = 1/(2\pi C\sqrt{R_1 R_2})$. For Butterworth $Q = 1/\sqrt{2}$: $R_1 = R_2 = R$. Then $f_c = 1/(2\pi RC) = 500$\,Hz, so $R = 1/(2\pi \times 500 \times 10 \times 10^{-9}) = 31.83\,\text{k}\Omega$. Use the nearest standard value: $R_1 = R_2 = 31.6\,\text{k}\Omega$ (1\% E96 series), giving $f_c = 503$\,Hz.
\item The ratio $f_{\text{stop}}/f_{\text{pass}} = 5000/500 = 10$ (one decade). Butterworth attenuation at $f_{\text{stop}}$: $A = -20n\log_{10}(f_{\text{stop}}/f_c)$\,dB. With $f_c \approx f_{\text{pass}} = 500$\,Hz: $A = -20n\log_{10}(10) = -20n$\,dB. For $-60$\,dB: $n \geq 3$. A 3rd-order Butterworth gives exactly $-60$\,dB at 5\,kHz. Butterworth is the best choice here because the signal bandwidth (500\,Hz) is well below Nyquist (5\,kHz), maximally flat passband preserves signal fidelity, and there is no requirement for waveform shape preservation (which would favor Bessel). Chebyshev would allow a lower order but introduces passband ripple that is unnecessary given the comfortable frequency margin.
\item $H(s) = 1000/(s + 1000)$ is a first-order LP with $\omega_c = 1000$\,rad/s, so $f_c = 1000/(2\pi) = 159.2$\,Hz. Passband gain: $|H(0)| = 1$ (0\,dB). At $f_c$: $|H| = -3$\,dB. Above $f_c$: roll-off at $-20$\,dB/decade. Phase: $0^{\circ}$ at DC, $-45^{\circ}$ at $f_c$, approaching $-90^{\circ}$ at high frequency.
\item (a) $f_c = 1/(2\pi \times 10{,}000 \times 100 \times 10^{-9}) = 159.2$\,Hz. (b) $\tau = RC = 10{,}000 \times 100 \times 10^{-9} = 1$\,ms. (c) $0.63\,V$ is reached at $t = \tau = 1$\,ms (by definition of the time constant). (d) At $10 f_c$: attenuation $= -20\log_{10}(\sqrt{1 + 10^2}) = -20.04$\,dB $\approx -20$\,dB.
\item (a) Passive is more appropriate for a battery-powered logger: no power consumption, no supply rail limitations, and a simple first-order RC suffices for anti-aliasing if placed close to the ADC input. (b) Active filter is required because passive filters can only attenuate, never amplify. An active Sallen-Key or MFB topology can provide gain. (c) Active filter is more appropriate: a 4th-order requires two cascaded Sallen-Key stages. Passive 4th-order filters suffer from impedance loading between stages and require buffer amplifiers anyway. With $\pm$15\,V supplies available, active filters provide clean performance.
\item (a) Butterworth: $n \geq \log_{10}(10^{40/20} - 1)/(2\log_{10}(2000/500))$. At $f = 2000$\,Hz, attenuation $= -20n\log_{10}(2000/f_c)$. With $f_c$ chosen so that attenuation at 500\,Hz is $-1$\,dB: $f_c \approx 580$\,Hz (from $-1 = -20n\log_{10}(500/580)$ iteration). Simpler estimate: $n \geq 40/(20\log_{10}(2000/500)) = 40/12.04 = 3.32$, so $n = 4$. (b) Chebyshev Type~I with 0.5\,dB ripple achieves sharper roll-off: $n = 3$ suffices. The Chebyshev is recommended if 0.5\,dB passband ripple is acceptable, as it requires fewer components (3 vs.\ 4 poles).
\end{enumerate}
