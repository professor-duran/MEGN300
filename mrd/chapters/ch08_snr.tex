...
\item \textbf{Separate analog/digital grounds}: digital switching noise must not contaminate analog signal paths. Connect grounds only at the star point.
...
\item \textbf{Decoupling}: 100\,nF ceramic at every IC power pin, 10\,$\mu$F electrolytic at each rail entry point.
\end{enumerate}

\begin{examplebox}[ThermoRig: Diagnosing 60\,Hz Noise]
The ThermoRig thermocouple signal shows a clean 25\,\degC{} reading on the benchtop but develops $\pm$0.3\,\degC{} oscillation when the heater is powered on. FFT reveals a dominant 60\,Hz peak with harmonics at 120, 180, 240\,Hz. Root cause: the heater power cable runs parallel to the thermocouple extension wire for 50\,cm, coupling 60\,Hz capacitively. Fix: (1) reroute the thermocouple wire away from the heater cable, (2) switch the DAQ to differential input mode, (3) add a 50-point moving average in LabVIEW (effective cutoff well below 60\,Hz given the slow thermal process). Result: noise drops to $\pm$0.02\,\degC{}. Cost: \$0 and 10 minutes.
\end{examplebox}
...
\textbf{Thermal noise}: $V_n = \sqrt{4k_B T R B}$. At 300\,K, a 10\,k$\Omega$ resistor over 10\,kHz: $V_n = 1.28$\,$\mu$V. This is the fundamental noise floor.
