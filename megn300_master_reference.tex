...
\end{tikzpicture}
\caption{Wheatstone bridge circuit. $R_4$ is the sensor element (e.g., RTD or strain gauge). When balanced ($R_1 R_4 = R_2 R_3$), $V_{\text{out}} = 0$. A change in sensor resistance produces a differential output proportional to $\Delta R/R$, measured by an instrumentation amplifier.}
...
\end{tikzpicture}
\caption{Wheatstone bridge configurations. Sensitivity doubles with each step: quarter-bridge ($1$ active element), half-bridge ($2$), full-bridge ($4$). Full bridge also provides complete temperature compensation.}
\label{fig:bridge}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{First-order system: step response (left) and Bode magnitude (right). The time constant $\tau$ determines both settling time ($5\tau$ to 99\%) and bandwidth ($f_{-3\text{dB}} = 1/2\pi\tau$).}
...
\label{ch:error}\index{uncertainty!propagation}\index{RSS}\index{GUM}
...
\end{tikzpicture}
\caption{RSS error budget for a temperature measurement system. The sensor dominates; improving the ADC has diminishing returns. Always improve the dominant contributor first.}
...
\label{ch:snr}\index{signal-to-noise ratio}\index{SNR}\index{noise!sources}
...
% Key insight box
\node[draw=MinesBlue, thick, fill=MinesBlue!5, rounded corners=3pt, text width=12cm,
...
\node[font=\tiny\sffamily\itshape, WarnOrange] at (axis cs:70,-15) {Energy smeared\\across bins};
\end{axis}
\begin{axis}[
...
\node[font=\tiny\sffamily\itshape, AccentTeal] at (axis cs:70,-15) {Energy concentrated\\in narrow peak};
\end{axis}
\end{tikzpicture}
...
\label{ch:dsp}\index{aliasing}\index{Nyquist theorem}\index{sampling}\index{oversampling}
...
\label{ch:analog_filters}\index{filter!low-pass}\index{filter!Butterworth}\index{filter!Chebyshev}\index{filter!Bessel}\index{cutoff frequency}\index{Sallen-Key}
...
\end{scope}
\end{tikzpicture}
\caption{Left: first-order passive RC low-pass filter. Right: Bode magnitude response comparing 1st order ($-20$\,dB/dec) and 2nd order ($-40$\,dB/dec). A first-order filter is often insufficient for anti-aliasing.}
\label{fig:rc_bode}
\end{figure}
...
% 4th-order Bessel (gentler rolloff)
...
\end{axis}
\end{tikzpicture}
\caption{Bode magnitude comparison of 4th-order Butterworth, Chebyshev, and Bessel low-pass filters. Chebyshev has the steepest rolloff but passband ripple. Bessel has the gentlest rolloff but preserves pulse shape (linear phase). Butterworth is the default.}
...
$y[n] = \sum b_k x[n\!-\!k] - \sum a_k y[n\!-\!k]$\\[4pt]
...
\end{tikzpicture}
\caption{Voltage divider loading error. An unloaded divider (left) produces the expected $V_{\text{out}}$. Connecting a load $R_L$ comparable to $R_2$ (right) changes the output because $R_2 \| R_L < R_2$. The 100:1 impedance rule ensures loading error stays below 1\%.}
...
\node[font=\tiny\sffamily\itshape, AccentTeal\bfseries] at (12,-1.5) {Bridge circuits};
\end{tikzpicture}
...
\label{ch:highpower}\index{MOSFET}\index{relay}\index{SSR}\index{flyback diode}\index{PWM}\index{H-bridge}
...
\node[font=\scriptsize\sffamily\bfseries, WarnOrange] at (4.5,4.0) {Flyback\\diode};
% MOSFET (simplified)
...
Once power is verified, inject a known signal at the input and trace it through each stage of the signal chain with an oscilloscope or multimeter. At each node, compare the measured signal to the expected value (calculated from the circuit design). When the measured value deviates from the expected value, you have localized the fault to the stage between the last good node and the first bad node.
...
For digital communication debugging (SPI, I2C, UART). Captures multiple digital signals simultaneously and decodes protocol frames. Many affordable USB logic analyzers ($<$\$20) work with free software (PulseView/sigrok). Use when: I2C devices are not responding, SPI data is corrupted, UART baud rate mismatch is suspected.
...
\end{tikzpicture}
\caption{Standard feedback control loop. The summing junction computes error $e = \text{SP} - \text{PV}$. Disturbances enter at the plant but the feedback loop detects and corrects their effect.}
...
% ================================================================
\chapter{PID Control}
\label{ch:pid}\index{PID control}\index{Ziegler-Nichols}\index{integral windup}
...
\draw[arr] (sensor) -| (sum);
\end{tikzpicture}
\caption{PID controller block diagram. The error $e(t) = r(t) - y_{\text{measured}}(t)$ drives three parallel paths: proportional (speed), integral (accuracy), derivative (damping). The sensor feedback path introduces its own dynamics and noise.}
\label{fig:pid_block}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{Step response comparison: P-only has steady-state offset, PI eliminates offset but overshoots, PID provides fast settling with minimal overshoot. Tune P first, then I, then D.}
\label{fig:pid_steps}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{Integral windup: without anti-windup (left), the integral accumulates during saturation, causing 80\%+ overshoot. With anti-windup clamping (right), the integral is frozen during saturation, limiting overshoot to $\sim$12\%. Always enable anti-windup.}
\label{fig:windup}
\end{figure}
...
\end{equation}
This conservation of energy equation connects pressure, velocity, and elevation. It is the foundation of many flow measurement devices: Venturi meters, orifice plates, and Pitot tubes all create a velocity change that produces a measurable pressure difference.
...
\textbf{Actuators}: hydraulic cylinders (linear motion) and hydraulic motors (rotary motion). A 50\,mm bore cylinder at 200\,bar produces $F = P \times A = 200 \times 10^5 \times (\pi/4)(0.05)^2 = 39.3$\,kN ($\sim$8800\,lbf) from a compact package.
...
Hydraulic systems operate at pressures that can cause severe injury or death. A 200\,bar (3000\,psi) pinhole leak produces a jet that can penetrate skin and inject fluid into tissue (fluid injection injury, a surgical emergency). Never use your hand to check for leaks; use a piece of cardboard. Always relieve pressure before disconnecting fittings. Wear safety glasses when working near pressurized lines.
\end{warningbox}
...
A \textbf{control valve} is the ``final control element'' in a feedback loop that regulates flow, pressure, or level. The PID controller (Chapter~17) sends a command signal (typically 4--20\,mA or 0--10\,V), and the valve adjusts its opening to achieve the desired flow rate.
...
\item $A_o = (\pi/4)(0.01)^2 = 7.85 \times 10^{-5}$\,m$^2$. $Q = C_d A_o \sqrt{2\Delta P/\rho} = 0.62 \times 7.85 \times 10^{-5} \times \sqrt{2 \times 15000/998} = 4.87 \times 10^{-5} \times 5.48 = 2.67 \times 10^{-4}$\,m$^3$/s $= 16.0$\,L/min.
...
\midrule
Temperature & Thermocouple (K) & $-200$ to $1250$\,\degC{} & $\pm 2.2$\,\degC{} & $\mu$V (needs amp) \\
...
Bessel & Maximally flat group delay & Gentlest rolloff & Preserving waveform shape \\
...
\midrule
K & $-200$ to $1250$ & 41 & Most common, general purpose \\
J & $-40$ to $750$ & 52 & Higher sensitivity, oxidizes above 500\,\degC \\
T & $-200$ to $350$ & 43 & Best for cryogenic and food applications \\
E & $-200$ to $900$ & 68 & Highest sensitivity of common types \\
N & $-270$ to $1300$ & 39 & Improved K-type, better stability \\
...
\begin{center}
\begin{longtable}{p{0.8cm}p{5.5cm}p{5.5cm}}
\toprule
