...
\begin{protip}
\textbf{The INA128 is your best friend for bridge circuits.} It has $>$10\,G$\Omega$ balanced input impedance, $>$100\,dB CMRR, and gain set by a single resistor ($G = 1 + 50\,\text{k}\Omega/R_G$. For $G=100$: $R_G = 50\,\text{k}\Omega/99 = 505\,\Omega$ (use 510\,$\Omega$ standard)). Whenever you measure a Wheatstone bridge (strain gauge, RTD, load cell), reach for an instrumentation amplifier first.
\end{protip}
...
\begin{protip}
\textbf{Check sensor output compatibility before ordering.} Verify three things: (1) the output voltage range fits your ADC input range (a 10\,V sensor on a 3.3\,V ADC will saturate and may damage the ADC), (2) the output impedance is compatible ($>$100:1 ratio with your ADC input impedance), and (3) the response time is fast enough for your signal ($\tau_{\text{sensor}} < f_{\text{signal}}/5$). Chapter~6 (Sensors and Actuators) has the sensor selection decision framework.
\end{protip}
...
INA128 & $G = 1 + 50\text{k}\Omega/R_G$; CMRR $>$ 100\,dB; Input $Z > 10$\,G$\Omega$ \\
